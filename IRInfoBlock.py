...
def Print_C_String():
    ...
    # IRInfoBlock
    # BlockHeader, Padding* a, IRDataUUID, Padding a, NumberOfIOCRs b, [(IOCRReference,
    # SubframeOffset, SubframeData)*]
    ...
    # Padding*: fixed at 2 Octets
    profinet_data.extend(['0x00', '0x00'])

    # IRDataUUID
    # Assumption is made that it is 16 bytes, no data could be found with the display filter in any of the available Pcaps.
    profinet_data.extend(['0xde', '0xa0', '0x00', '0x00', '0x6c', '0x97', '0x11', '0xd1', '0x82', '0x71', '0x00', '0x01', '0x00', '0x01', '0x01', '0x74'])
    # profinet_data.extend(['0xa3, 0x5b, 0x6c, 0x17, 0x8a, 0xfd, 0x23, 0x48, 0x9c, 0x0e, 0x58, 0x12, 0xf4, 0x7d, 0x90, 0x66'])

    # Padding*: fixed at 2 Octets. It is additional to the previous one.
    profinet_data.extend(['0x00', '0x00'])

    # Coding of the field NumberOfIOCRs
    # This field shall be coded as data type Unsigned16.
    profinet_data.extend(['0x00', '0x01'])

    # Coding of the field IOCRReference
    # This field shall be coded as data type Unsigned16. It is an identification tag for the CR and is
    # used within the IOCRBlockReq and IOCRBlockRes to reference the DataItem. Furthermore, it
    # is used in PDIRData of the Physical Device ASE.
    profinet_data.extend(['0x00', '0x01'])

    # Coding of the field SubframeOffset
    # This field shall be coded as data type Unsigned16 with values according to Table 1001.
    # First acceptable value is chosen.
    profinet_data.extend(['0x00', '0x06'])

    # Coding of the field SubframeData
    # The value is being set according to the bitwise information given.
    # Bit 0-6: 0x01 ->1st acceptable value.
    # profinet_data.append('0x01')
    # Bit 7 Reserved:
    # Bit 8-15: 0x01 -> 1st acceptable value
    # profinet_data.append('0x01')
    # Bit 16-31: 0x01 -> 1st acceptable value
