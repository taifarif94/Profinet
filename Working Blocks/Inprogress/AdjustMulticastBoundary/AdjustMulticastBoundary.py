...
def Print_C_String():
    ...
    # Adjust Multicast Boundary
    # BlockHeader, Padding, Padding, MulticastBoundary, AdjustProperties, [Padding*] a
    ...
    with open('AdjustMulticastBoundary.txt', 'w') as f:
        for i in range(0, len(profinet_data), 8):
            f.write(', '.join(profinet_data[i:i + 8]) + ',\n')
    # Now, remove the last comma and newline
    with open('AdjustMulticastBoundary.txt', 'rb+') as f:  # note the mode 'rb+'
