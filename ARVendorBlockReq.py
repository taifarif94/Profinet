...
def Print_C_String():
    ...
    # BlockType: 0x0108
    profinet_data.extend(['0x01', '0x08'])
    # BlockLength
    ...
    # APStructureIdentifier
    # Coded as Unsigned16
    # When the APStructureIdentifier is 0x8000, AND the API is :=0, this calls for a special structure.
    # So, The caseChecks, checks the condition for the APStructureIdentifier
    # Highest acceptable Administrative number which works for both API =0 and API !=0 ia being used.

    caseCheckStructure = ['0xFF','0xFF']
    profinet_data.extend(caseCheckStructure)

    # API
    # Coded as  Unsigned32
    # Default value of 0 is taken
    caseCheckAPI = ['0x00','0x00','0x00','0x00']

    # Coding value for the extended ExpectedExtendedIdentificationInfo is not given so the condition is
    # being commented for now
    # if caseCheckStructure == ['0x80','0x00'] and caseCheckAPI ==['0x00','0x00','0x00','0x00']:

    # Codin info for the data field is not given therefore, it is being omitted for the time being. It is an optional field.
    # Possibly the APStructureIdentifier defines this field but how to decode?
    ...
    with open('Data/ARVendorBlockReq_alarm_1325.txt', 'w') as f:
