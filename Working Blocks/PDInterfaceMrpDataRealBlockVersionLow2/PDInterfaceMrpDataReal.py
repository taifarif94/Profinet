...
def Print_C_String():
    ...
    # MrpInstance-DataRealBlock
    BlockLengthStartInner = len(profinet_data)

    # BlockHeader BlockType, BlockLength, BlockVersionHigh, BlockVersionLow
    # BlockType: 0x0232
    profinet_data.extend(['0x02', '0x32'])
    # BlockLength
    # 0x0003 – 0xFFFF Number of octets without counting the fields BlockType and BlockLength
    profinet_data.extend(['0x00', '0x00'])

    # BlockLengthIndex
    BlockLengthIndexInner = len(profinet_data) - 2
    ...
    # Assigning the correct RealIdentificationData BlockLength: 36 (0x0024)
    # This is Minus 4 because The block length includes everything in the block except the block type and itself which is 4 bytes
    IODReadResHeaderRecordDataLength = (BlockLengthEnd - BlockLengthStartInner) - 4
    profinet_data[BlockLengthIndexInner] = '0x' + format(IODReadResHeaderRecordDataLength, '04x')[0:2]
    profinet_data[BlockLengthIndexInner + 1] = '0x' + format(IODReadResHeaderRecordDataLength, '04x')[2:4]
    print(IODReadResHeaderRecordDataLength)
