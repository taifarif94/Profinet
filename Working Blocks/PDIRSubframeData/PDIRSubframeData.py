...
def Print_C_String():
    ...
    # PDIRSubframeData
    # BlockHeader, NumberOfSubframeBlocks, SubframeBlock*
    ...
    # BlockType: 0x022A
    profinet_data.extend(['0x02', '0x2a'])
    # BlockLength
    ...
    BlockInnerLengthStart = len(profinet_data)

    # BlockHeader BlockType, BlockLength, BlockVersionHigh, BlockVersionLow
    # BlockType: 0x022B
    profinet_data.extend(['0x02', '0x2b'])
    # BlockLength
    # 0x0003 – 0xFFFF Number of octets without counting the fields BlockType and BlockLength
    profinet_data.extend(['0x00', '0x00'])

    # BlockLengthIndex
    BlockInnerLengthIndex = len(profinet_data) - 2
    ...
    # FrameID: 0x80f8
    # Random Value Chosen / No description in Field Coding
    profinet_data.extend(['0x80', '0xF8'])

    # SFIOCRProperties
    # Bit specifications as per the manual
    profinet_data.extend(['0x00', '0x00', '0x0A', '0x03'])
    ...
    block_length_current = BlockLengthEnd - BlockInnerLengthStart
    ...
    # Assigning the correct SubFrame BlockLength
    # This is Minus 4 because The block length includes everything in the block except the block type and itself which is 4 bytes
    SubFrameBlockLength = (BlockLengthEnd - BlockInnerLengthStart) - 4
    profinet_data[BlockInnerLengthIndex] = '0x' + format(SubFrameBlockLength, '04x')[0:2]
    profinet_data[BlockInnerLengthIndex + 1] = '0x' + format(SubFrameBlockLength, '04x')[2:4]
    print(SubFrameBlockLength)
