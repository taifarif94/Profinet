...
def Print_C_String():
    ...
    RealIdentificationDataLengthStart = len(profinet_data)
    ...
    # RealIdentificationBlock BlockLength
    RealIdentificationBlockLengthIndex = len(profinet_data)-2
    ...
    # profinet_data.append('0x00000000')
    profinet_data.extend(['0x00', '0x00', '0x00', '0x00'])
    # NumberOfSlots
    # Coded as data type Unsigned16.
    # profinet_data.append('0x0001')
    ...
    # ModuleIdentNumber
    # Coded as data type Unsigned32
    # profinet_data.append('0x00000300')
    profinet_data.extend(['0x00', '0x00','0x03','0x00'])
    ...
    profinet_data.extend(['0x00', '0x00', '0x80', '0x00'])

    # SubSlotNumber
    # Coded as data type Unsigned16.
    # SubslotNumber: 0x8001
    profinet_data.extend(['0x80', '0x01'])

    # SubmoduleIdentNumber
    # Coded as data type Unsigned32.
    # SubmoduleIdentNumber: 0x00008001
    profinet_data.extend(['0x00', '0x00', '0x80', '0x01'])

    # Stub data/ Fragment length End marker.
    fragment_length_end_marker = len(profinet_data.copy())

    # RealIdentification Block Length End
    RealIdentificationDataLengthEnd = len(profinet_data)
    ...
    # Assigning the correct Actual Count in Profinet IO Device (Read)
    IODReadResHeaderRecordDataLength = RealIdentificationDataLengthEnd-RealIdentificationDataLengthStart
    ...
    IODReadResHeaderRecordDataLength = (RealIdentificationDataLengthEnd - RealIdentificationDataLengthStart)-4
    profinet_data[RealIdentificationBlockLengthIndex] = '0x' + format(IODReadResHeaderRecordDataLength, '04x')[0:2]
    profinet_data[RealIdentificationBlockLengthIndex + 1] = '0x' + format(IODReadResHeaderRecordDataLength, '04x')[2:4]
    ...
    with open('Data/RealIdentificationLow_RealIdent_167.txt', 'w') as f:
        for i in range(0, len(profinet_data), 8):
            f.write(', '.join(profinet_data[i:i + 8]) + ',\n')
    # Now, remove the last comma and newline
    with open('Data/RealIdentificationLow_RealIdent_167.txt', 'rb+') as f:  # note the mode 'rb+'
